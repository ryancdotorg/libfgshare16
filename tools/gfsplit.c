...
{
  FILE *inputfile;
  unsigned short* sharenrs = malloc( sharecount * sizeof(short));
  unsigned int i, j;
  ...
  for( i = 0; i < sharecount; ++i ) {
    unsigned short proposed = random();
    if( proposed == 0 ) {
      proposed = 1;
    }
    SHARENR_TRY_AGAIN:
    ...
    sharenrs[i] = proposed;
    sprintf( outputfilebuffer, "%s.%05d", _outputstem, proposed );
  ...
  G = gfshare_ctx_init_enc( sharenrs, sharecount, 
                            threshold, MIN(BUFFER_SIZE, getlen( inputfile )) / 2 );
  if( !G ) {
  ...
  while( !feof(inputfile) ) {
    ...
    gfshare_ctx_enc_setsecret( G, (unsigned short *)buffer );
    for( i = 0; i < sharecount; ++i ) {
      unsigned int bytes_written;
      gfshare_ctx_enc_getshare( G, i, (unsigned short *)buffer );
