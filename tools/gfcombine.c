...
number which will be taken to be the share number. I.E. \".NNNNN\".\n\
", progname );
}
...
{
  fprintf( stderr, "%s: %s: bad filename\nInput files should be called <name>.NNNNN\n", progname, fname );
}
...
check_filenames( char **filenames, int count )
{
  ...
  for( i = 0; i < count; ++i ) {
    ...
    if( filenames[i][nlen-6] != '.' ||
        !isdigit(filenames[i][nlen-5]) ||
        !isdigit(filenames[i][nlen-4]) ||
    ...
    if( filenames[i][nlen-5] == '0' &&
        filenames[i][nlen-4] == '0' &&
        filenames[i][nlen-3] == '0' &&
...
do_gfcombine( char *outputfilename, char **inputfilenames, int filecount )
{
  ...
  unsigned short* sharenrs = malloc( filecount * sizeof(short));
  int i;
  ...
  for( i = 0; i < filecount; ++i ) {
    ...
    sharenrs[i] = strtoul( inputfilenames[i] + strlen(inputfilenames[i]) - 5, 
                           NULL, 10 );
  ...
  G = gfshare_ctx_init_dec( sharenrs, filecount, BUFFER_SIZE/2 );
  
  while( !feof(inputfiles[0]) ) {
    unsigned int bytes_read = fread( buffer, 1, BUFFER_SIZE, inputfiles[0] );
    unsigned int bytes_written;
    gfshare_ctx_dec_giveshare( G, 0, (unsigned short*)buffer );
    for( i = 1; i < filecount; ++i ) {
      ...
      gfshare_ctx_dec_giveshare( G, i, (unsigned short*)buffer );
    }
    gfshare_ctx_dec_extract( G, (unsigned short*)buffer );
